...
import { createAdminLocalComputeTask, getAdminLocalComputeSnapshot, getPackages, registerAdminLocalComputeNode, updateAdminLocalComputeNodeSharePolicy } from "@/lib/api";
...
export function AdminLocalCompute() {
  ...
  const [shareForms, setShareForms] = useState<Record<string, {
  ...
  async function handleSaveSharePolicy(nodeId: string) {
    if (!session?.csrfToken) return;
    const form = shareForms[nodeId];
    if (!form) return;
    setMessage("");
    setError("");
    try {
      ...
      setMessage(`Updated sharing policy for ${nodeId}.`);
      await loadData();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Failed to update sharing policy");
    }
  }
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <select className="h-10 w-full rounded-md border border-slate-700 bg-[#0a0e17] px-3 text-sm text-slate-200" value={localComputeForm.sharingMode} onChange={(e) => setLocalComputeForm((current) => ({ ...current, sharingMode: e.target.value === "trusted-shared" ? "trusted-shared" : "author-only" }))}>
              <option value="author-only">Author only</option>
              <option value="trusted-shared">Trusted shared runtime</option>
            </select>
            <textarea className="min-h-20 w-full rounded-md border border-slate-700 bg-[#0a0e17] px-3 py-2 text-sm text-slate-200" placeholder="Trusted user emails" value={localComputeForm.sharedWithEmails} onChange={(e) => setLocalComputeForm((current) => ({ ...current, sharedWithEmails: e.target.value }))} />
            <textarea className="min-h-16 w-full rounded-md border border-slate-700 bg-[#0a0e17] px-3 py-2 text-sm text-slate-200" placeholder="Allowed path scopes" value={localComputeForm.allowedPathScopes} onChange={(e) => setLocalComputeForm((current) => ({ ...current, allowedPathScopes: e.target.value }))} />
            <textarea className="min-h-16 w-full rounded-md border border-slate-700 bg-[#0a0e17] px-3 py-2 text-sm text-slate-200" placeholder="Allowed auth capabilities, e.g. consent:gemini-web" value={localComputeForm.allowedAuthCapabilities} onChange={(e) => setLocalComputeForm((current) => ({ ...current, allowedAuthCapabilities: e.target.value }))} />
          ...
          <CardContent className="space-y-3">
            {(snapshot?.nodes || []).map((node) => (
              <div key={node.nodeId} className="rounded-2xl border border-slate-800 bg-slate-950/40 px-4 py-4">
                ...
                <div className="mt-2 text-xs text-slate-500">{node.sharingMode === "trusted-shared" ? "trusted shared runtime" : "author only"} · shared users {(node.sharedWithUsers || []).length}</div>
                {node.ownerEmail ? <div className="mt-1 text-xs text-slate-500">owner {node.ownerEmail}</div> : null}
                {node.lastError ? <div className="mt-2 text-xs text-red-300">{node.lastError}</div> : null}
                <div className="mt-3 grid gap-2">
                  <select className="h-10 w-full rounded-md border border-slate-700 bg-[#0a0e17] px-3 text-sm text-slate-200" value={shareForms[node.nodeId]?.sharingMode || "author-only"} onChange={(e) => setShareForms((current) => ({ ...current, [node.nodeId]: { ...(current[node.nodeId] || { sharingMode: "author-only", sharedWithEmails: "", allowedPathScopes: "", allowedAuthCapabilities: "" }), sharingMode: e.target.value === "trusted-shared" ? "trusted-shared" : "author-only" } }))}>
                    <option value="author-only">Author only</option>
                    <option value="trusted-shared">Trusted shared runtime</option>
                  </select>
                  <textarea className="min-h-20 w-full rounded-md border border-slate-700 bg-[#0a0e17] px-3 py-2 text-sm text-slate-200" placeholder="Trusted user emails" value={shareForms[node.nodeId]?.sharedWithEmails || ""} onChange={(e) => setShareForms((current) => ({ ...current, [node.nodeId]: { ...(current[node.nodeId] || { sharingMode: "author-only", sharedWithEmails: "", allowedPathScopes: "", allowedAuthCapabilities: "" }), sharedWithEmails: e.target.value } }))} />
                  <textarea className="min-h-16 w-full rounded-md border border-slate-700 bg-[#0a0e17] px-3 py-2 text-sm text-slate-200" placeholder="Allowed path scopes" value={shareForms[node.nodeId]?.allowedPathScopes || ""} onChange={(e) => setShareForms((current) => ({ ...current, [node.nodeId]: { ...(current[node.nodeId] || { sharingMode: "author-only", sharedWithEmails: "", allowedPathScopes: "", allowedAuthCapabilities: "" }), allowedPathScopes: e.target.value } }))} />
                  <textarea className="min-h-16 w-full rounded-md border border-slate-700 bg-[#0a0e17] px-3 py-2 text-sm text-slate-200" placeholder="Allowed auth capabilities" value={shareForms[node.nodeId]?.allowedAuthCapabilities || ""} onChange={(e) => setShareForms((current) => ({ ...current, [node.nodeId]: { ...(current[node.nodeId] || { sharingMode: "author-only", sharedWithEmails: "", allowedPathScopes: "", allowedAuthCapabilities: "" }), allowedAuthCapabilities: e.target.value } }))} />
                  <Button variant="outline" className="border-slate-700 text-slate-200 hover:bg-slate-800" onClick={() => void handleSaveSharePolicy(node.nodeId)}>
                    Save Sharing Policy
                  </Button>
                </div>
              </div>
