...
import { readAgentOsConfig } from "./agentOsConfig";
import { listIndexedSkills } from "./skillIndex";
import { getRepoRoot } from "./workspaceTopology";
...
const externalKnowledgeRoot = path.join(libraryRoot, "knowledge");
...
function readExternalKnowledgeFiles() {
  if (!exists(externalKnowledgeRoot)) {
    return [];
  }

  const files: string[] = [];
  const includeRoots = [
    path.join(externalKnowledgeRoot, "projects"),
    path.join(externalKnowledgeRoot, "shared"),
    path.join(externalKnowledgeRoot, "_taxonomy"),
  ];
...
function buildExternalLibraryKnowledge(): KnowledgeItem[] {
  return readExternalKnowledgeFiles().map((docPath) => {
    const markdown = readTextIfExists(docPath) ?? "";
    const frontmatter = parseFrontmatter(markdown);
    const title = firstHeading(markdown)?.replace(/^#\s*/, "") || path.basename(docPath, ".md");
    const summary = firstParagraph(markdown) || "External library knowledge document";
    const relative = path.relative(externalKnowledgeRoot, docPath);
    const knowledgeType = knowledgeTypeFromAgentPath(relative, frontmatter);
    const evidenceLevel = evidenceFromFrontmatter(frontmatter);
    ...
    return {
      ...
      human: {
        ...
        summary,
        content_md: markdown.slice(0, 6000),
        tags: ["ai-library", ...relative.split(path.sep).slice(0, -1)].filter(Boolean),
        domain: "AI Library",
      ...
      machine: {
        ...
        entities: { docPath, relativePath: relative },
        steps: extractBulletLines(extractSection(markdown, "Steps")),
        commands: extractCommands(markdown),
        constraints: extractBulletLines(extractSection(markdown, "Constraints")),
      },
    } satisfies KnowledgeItem;
  });
