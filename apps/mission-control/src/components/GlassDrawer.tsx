...
import { executeNode, exportPortableBundle, fetchInputProfiles, saveInputProfile, searchKnowledge } from "../services/api";
...
export const GlassDrawer = ({ isOpen, onClose, node }: GlassDrawerProps) => {
  ...
  const [inputProfiles, setInputProfiles] = useState<Array<{ id: string; name: string; values: Record<string, string> }>>([]);
  ...
  React.useEffect(() => {
    if (!node?.drawerContent || !isOpen) {
      return;
    }
    let cancelled = false;
    fetchInputProfiles(node.id)
      .then((payload) => {
        if (cancelled) return;
        setInputProfiles(payload.profiles ?? []);
        const defaultProfile = (payload.profiles ?? []).find((item) => item.id === payload.defaultProfileId);
        if (defaultProfile) {
  ...
  const derivedProfileName =
    inputValues["邮箱地址"]?.trim() ||
    inputValues["账户"]?.trim() ||
    inputValues["项目名"]?.trim() ||
    label;
                          ...
                          <div className="flex gap-2">
                            <select
                              ...
                              onChange={(event) => {
                                ...
                                const selected = inputProfiles.find((item) => item.id === nextId);
                                if (selected) {
                                  setInputValues((current) => ({
                                    ...current,
                                    ...selected.values,
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  const payload = await saveInputProfile({
                                    nodeId: node.id,
                                    profileId: selectedProfileId || undefined,
                                    name: derivedProfileName,
                                    values: inputValues,
                                    setDefault: true,
                                  });
                                  setInputProfiles(payload.profiles);
                                  if (payload.defaultProfileId) {
                                    setSelectedProfileId(payload.defaultProfileId);
                                  }
                                  setDrawerFeedback(`Saved profile: ${derivedProfileName}`);
                                } catch (error) {
                                  console.error("Failed to save profile", error);
                                  setDrawerFeedback("Failed to save profile.");
                                } finally {
