...
export const DecisionQueuePanel = React.memo(function DecisionQueuePanel({
  decisions,
  liveSummary,
  ...
  const localizeTitle = (title: string) => {
    const map: Record<string, string> = {
      ...
      "账号研究待运行": t("shortVideoFactory.action.runResearch"),
      "是否纳入生产": t("shortVideoFactory.action.generateBrief"),
      "NotebookLM 增强归纳": t("shortVideoFactory.action.runNotebooklm"),
      "导演方案确认": t("shortVideoFactory.action.runProduction"),
      "Gemini 增强分析待确认": t("shortVideoFactory.warning.title"),
  ...
  return (
    ...
    >
      ...
      {liveSummary && (
        <div
          className="grid grid-cols-2 gap-2 rounded-2xl border px-3 py-3 md:grid-cols-4"
          style={{
        ...
        >
          <div>
            <div className="text-[10px]" style={{ color: "var(--text-secondary)" }}>
              {t("decisionQueue.live.agent")}
            </div>
            <div className="mt-1 text-xs font-semibold" style={{ color: "var(--text-primary)" }}>
              {liveSummary.agentOnline
                ? t("decisionQueue.live.agentOnline")
                : t("decisionQueue.live.agentWaiting")}
            </div>
          </div>
          <div>
            <div className="text-[10px]" style={{ color: "var(--text-secondary)" }}>
              {t("decisionQueue.live.active")}
            </div>
            <div className="mt-1 text-xs font-semibold" style={{ color: "var(--text-primary)" }}>
              {liveSummary.activeCount}
            </div>
          </div>
          <div>
            <div className="text-[10px]" style={{ color: "var(--text-secondary)" }}>
              {t("decisionQueue.live.queued")}
            </div>
            <div className="mt-1 text-xs font-semibold" style={{ color: "var(--text-primary)" }}>
              {liveSummary.queuedCount}
            </div>
          </div>
          <div>
            <div className="text-[10px]" style={{ color: "var(--text-secondary)" }}>
              {t("decisionQueue.live.latest")}
            </div>
            <div className="mt-1 text-xs font-semibold line-clamp-1" style={{ color: "var(--text-primary)" }}>
              {liveSummary.latestResultLabel ?? t("decisionQueue.live.none")}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-3" role="list" aria-label="Decision queue items">
          {visibleDecisions.map((decision) => (
            ...
            >
              ...
              (decision.onSecondaryActionClick && decision.secondaryCtaLabel) ||
              (decision.onIgnoreClick && decision.ignoreLabel) ||
              (decision.onSnoozeClick && decision.snoozeLabel) ? (
                <div className="mt-3 flex flex-wrap justify-end gap-2">
                  {decision.onSnoozeClick && decision.snoozeLabel && (
                    <button
                      type="button"
                      onClick={decision.onSnoozeClick}
                      aria-label={`${decision.snoozeLabel} for ${decision.title}`}
                    ...
                    >
                      {decision.snoozeLabel}
                    </button>
                  )}
                  {decision.onIgnoreClick && decision.ignoreLabel && (
                    <button
                      type="button"
                      onClick={decision.onIgnoreClick}
                      aria-label={`${decision.ignoreLabel} for ${decision.title}`}
                      className="rounded-full border px-4 py-1 text-[11px] font-semibold transition hover:-translate-y-0.5"
                      style={{
                    ...
                    >
                      {decision.ignoreLabel}
                    </button>
                  )}
                  {decision.onSecondaryActionClick && decision.secondaryCtaLabel && (
                    <button
                      type="button"
                      onClick={decision.onSecondaryActionClick}
                      aria-label={`${decision.secondaryCtaLabel} for ${decision.title}`}
                      className="rounded-full border px-4 py-1 text-[11px] font-semibold transition hover:-translate-y-0.5"
                      style={{
                        borderColor: "var(--border-color)",
                        backgroundColor: "transparent",
                  ...
                  {decision.onActionClick && decision.ctaLabel && (
                    <button
                      ...
                      aria-label={`${decision.ctaLabel} for ${decision.title}`}
                      className="rounded-full border px-4 py-1 text-[11px] font-semibold transition hover:-translate-y-0.5"
                      style={{
